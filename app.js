...
// Sort by earliest arrival date for each occasion
function buildOccasionFilters() {
  const occasionMap = new Map(); // lowercase -> { display, earliestArrival }
  allDrops.forEach(drop => {
    // Use the holiday field as the source of truth
    if (drop.holiday) {
      const key = drop.holiday.toLowerCase();
      const arrivalDate = drop.arrival; // YYYY-MM-DD format
      if (!occasionMap.has(key)) {
        occasionMap.set(key, { display: drop.holiday, earliestArrival: arrivalDate });
      } else {
        // Update if this drop has an earlier arrival
        const existing = occasionMap.get(key);
        if (arrivalDate < existing.earliestArrival) {
          existing.earliestArrival = arrivalDate;
        }
      }
    }
  });
  ...
  // Sort by earliest arrival date, then alphabetically as tiebreaker
  const occasionList = [...occasionMap.values()]
    .sort((a, b) => a.earliestArrival.localeCompare(b.earliestArrival) || a.display.localeCompare(b.display))
    .map(o => o.display);
